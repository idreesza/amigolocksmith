  ...
  return (
    <section className="relative bg-[#0D1B33] py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          kicker="Why Keys on Wheels"
          title={
            <>
              The Difference Is <span className="text-gradient-amber">in the Details</span>
            </>
          }
          sub="Locksmith scams are everywhere — $19 callout ads that turn into $300 bills. We built Keys on Wheels to be the company we'd want to call."
        />
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {REASONS.map((r, i) => (
            <Reveal key={r.title} delay={(i % 3) * 0.1}>
              <div className="h-full rounded-3xl border border-slate-700/60 bg-slate-900/50 p-7 transition hover:border-amber-400/40 hover:bg-slate-900">
